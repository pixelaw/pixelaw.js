...
export class RenderingMethods {
	...
	public render(): void {
		...
		if (
			this.renderer.options.showGridLines &&
			getZoomLevel(this.renderer.zoom) !== "far"
		) {
			this.drawGridLines();
		}
	...
	private drawGridLines(): void {
		...
		for (let x = left; x <= right; x++) {
			const screenX =
				Math.floor(x * cellSize * this.renderer.zoom + this.renderer.offsetX) +
				0.5;
			this.renderer.ctx.beginPath();
			this.renderer.ctx.moveTo(
				screenX,
				top * cellSize * this.renderer.zoom + this.renderer.offsetY,
			);
			this.renderer.ctx.lineTo(
				screenX,
				(bottom + 1) * cellSize * this.renderer.zoom + this.renderer.offsetY,
			);
		...
		for (let y = top; y <= bottom; y++) {
			const screenY =
				Math.floor(y * cellSize * this.renderer.zoom + this.renderer.offsetY) +
				0.5;
			this.renderer.ctx.beginPath();
			this.renderer.ctx.moveTo(
				left * cellSize * this.renderer.zoom + this.renderer.offsetX,
				screenY,
			);
			this.renderer.ctx.lineTo(
				(right + 1) * cellSize * this.renderer.zoom + this.renderer.offsetX,
				screenY,
			);
			this.renderer.ctx.stroke();
		}
	...
	public drawNotifications(): void {
		for (const [
			key,
			notification,
		] of this.renderer.activeNotifications.entries()) {
			const [x, y] = key.split(",").map(Number);
			const screenX =
				x * this.renderer.options.cellSize * this.renderer.zoom +
				this.renderer.offsetX;
			const screenY =
				y * this.renderer.options.cellSize * this.renderer.zoom +
	...
	private drawCell(pixel: Pixel): void {
		...
		const screenX =
			pixel.x * cellSize * this.renderer.zoom + this.renderer.offsetX;
		const screenY =
			pixel.y * cellSize * this.renderer.zoom + this.renderer.offsetY;
		const screenSize = cellSize * this.renderer.zoom;

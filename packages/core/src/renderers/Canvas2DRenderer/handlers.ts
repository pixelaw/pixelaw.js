...
export class EventHandlers {
  ...
  public handleMouseMove(event: MouseEvent): void {
    ...
    if (timeDiff > 0) {
      this.renderer.panInertiaVelocity = [
        (deltaX / timeDiff) * 15,
        (deltaY / timeDiff) * 15,
      ];
  ...
  public handleMouseUp(event: MouseEvent): void {
    ...
    if (timeDiff > 10 && timeDiff < 1500 && totalDistance < 10) {
      ...
      const x = Math.floor(
        (mouseX - this.renderer.offsetX) /
          (this.renderer.zoom * this.renderer.options.cellSize),
      );
      const y = Math.floor(
        (mouseY - this.renderer.offsetY) /
          (this.renderer.zoom * this.renderer.options.cellSize),
      );
  ...
  public handleTouchStart(event: TouchEvent): void {
    if (event.touches.length === 1) {
      event.preventDefault();
      this.renderer.dragStartTime = Date.now();
      this.renderer.dragStartCoord = [
        event.touches[0].clientX,
        event.touches[0].clientY,
      ];
      this.renderer.lastDragPoint = [
        event.touches[0].clientX,
        event.touches[0].clientY,
      ];
    ...
    if (event.touches.length === 1 && this.renderer.dragStartTime) {
      ...
      const newZoom = Math.max(
        ...
        Math.min(
          ...
          this.renderer.initialPinchZoom * pinchRatio,
        ),
      );
      ...
      this.renderer.zoomInertiaVelocity =
        (newZoom / this.renderer.zoom - 1) * 0.1; // Adjust multiplier for desired speed
  ...
  public handleTouchEnd(event: TouchEvent): void {
    if (event.touches.length === 0) {
      ...
      const mouseEvent = {
        clientX: this.renderer.lastDragPoint[0],
        clientY: this.renderer.lastDragPoint[1],
      };

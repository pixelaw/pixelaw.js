...
export class PixelawCore {
	...
	public set wallet(wallet: Wallet | null) {
		...
		this.storage
			.setItem(this.getStorageKey("wallet"), JSON.stringify(wallet))
			.catch(console.error);
	...
	public async loadWorld(world: string, urlDefaults?: CoreDefaults) {
		...
		if (!Object.hasOwn(this._worldsRegistry, world))
		...
		const storedLastEventAck = await this.storage.getItem(
			this.getStorageKey("lastEventAck"),
		);
		if (storedLastEventAck) {
			this._lastEventAck = parseInt(String(storedLastEventAck), 10) || 0;
		}
	...
	public set app(newApp: string | null) {
		...
		this.storage
			.setItem(this.getStorageKey("app"), newApp)
			.catch(console.error);
	}
	...
	private set world(newWorld: string) {
		...
		this.storage
			.setItem(this.getStorageKey("world"), newWorld)
			.catch(console.error);
	...
	public set color(newColor: number | null) {
		...
		this.storage
			.setItem(this.getStorageKey("color"), newColor.toString())
			.catch(console.error);
	}
	...
	public set zoom(newZoom: number) {
		...
		this.storage
			.setItem(this.getStorageKey("zoom"), newZoom.toString())
			.catch(console.error);
	}
	...
	public set lastEventAck(newLastEventAck: number) {
		...
		if (
			!Number.isFinite(newLastEventAck) ||
			newLastEventAck < 1577836800000 ||
			newLastEventAck > 4102444800000
		)
			return;
	...
	public set center(newCenter: Coordinate) {
		...
		this.storage
			.setItem(this.getStorageKey("center"), JSON.stringify(newCenter))
			.catch(console.error);
	}

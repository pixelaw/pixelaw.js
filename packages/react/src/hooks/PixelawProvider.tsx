...
export const PixelawContext = createContext<IPixelawContext | undefined>(
	undefined,
);
...
export const PixelawProvider = ({
	...
	useEffect(() => {
		const handlers = {
			statusChanged: (newStatus: CoreStatus) =>
				setContextValues((prev) => ({ ...prev, coreStatus: newStatus })),
			walletChanged: (newWallet: Wallet | null) =>
				setContextValues((prev) => ({ ...prev, wallet: newWallet })),
			appChanged: (newApp: string | null) =>
				setContextValues((prev) => ({ ...prev, app: newApp })),
			engineChanged: (newEngine: Engine | null) =>
				setContextValues((prev) => ({ ...prev, engine: newEngine })),
			worldChanged: (newWorld: string) =>
				setContextValues((prev) => ({ ...prev, world: newWorld })),
			colorChanged: (newColor: number) =>
				setContextValues((prev) => ({ ...prev, color: newColor })),
			centerChanged: (newCenter: Coordinate) =>
				setContextValues((prev) => ({ ...prev, center: newCenter })),
			zoomChanged: (newZoom: number) =>
				setContextValues((prev) => ({ ...prev, zoom: newZoom })),
		};
...
export const usePixelawProvider = (): IPixelawContext => {
	...
	if (!context)
		throw new Error(
			"usePixelawProvider can only be used within a PixelawProvider",
		);
	return context;
};

...
export async function dojoInit(
	...
	const { apps, manifest, coreAddress } = await fetchAppsAndManifest(
		worldConfig,
		sdk,
	);
	...
	const burnerConnector = await setupBurnerConnector(
		provider,
		worldConfig,
		core.storage,
	);
...
async function fetchAppsAndManifest(
	...
	try {
		...
		const query =
			"SELECT internal_entity_id, name, system, action, icon FROM 'pixelaw-App';";
		...
		const coreActionsClass = await getClass(provider, coreActionsAddress);
		...
		contracts = contracts.concat(
			await Promise.all(apps.map((app) => getAbi(provider, app))),
		);
...
function setupControllerConnector(
	manifest: Manifest,
	worldConfig: DojoConfig,
...
async function setupBurnerConnector(
	...
	const cacheKey = JSON.stringify({
		rpcProvider,
		burnerConfig: worldConfig.wallets?.burner,
	});

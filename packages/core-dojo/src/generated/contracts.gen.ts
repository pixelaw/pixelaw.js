...
export function setupWorld(provider: DojoProvider) {
	...
	const build_player_actions_configure_calldata = (defaultParams: models.DefaultParameters, emoji: models.Emoji): DojoCall => {
		return {
	...
	const player_actions_configure = async (snAccount: Account | AccountInterface, defaultParams: models.DefaultParameters, emoji: models.Emoji) => {
		try {
			return await provider.execute(
				snAccount,
	...
	const build_player_actions_interact_calldata = (defaultParams: models.DefaultParameters): DojoCall => {
		return {
			contractName: "player_actions",
			entrypoint: "interact",
			calldata: [defaultParams],
	...
	const player_actions_interact = async (snAccount: Account | AccountInterface, defaultParams: models.DefaultParameters) => {
		try {
			return await provider.execute(
				snAccount,
	...
	const build_player_actions_onPostUpdate_calldata = (pixelUpdate: models.PixelUpdate, appCaller: models.App, playerCaller: string): DojoCall => {
		return {
			contractName: "player_actions",
			entrypoint: "on_post_update",
			calldata: [pixelUpdate, appCaller, playerCaller],
	...
	const player_actions_onPostUpdate = async (snAccount: Account | AccountInterface, pixelUpdate: models.PixelUpdate, appCaller: models.App, playerCaller: string) => {
		try {
			return await provider.execute(
				snAccount,
				build_player_actions_onPostUpdate_calldata(pixelUpdate, appCaller, playerCaller),
				"pixelaw",
			);
		} catch (error) {
			console.error(error);
	...
	const build_player_actions_onPreUpdate_calldata = (pixelUpdate: models.PixelUpdate, appCaller: models.App, playerCaller: string): DojoCall => {
		return {
			contractName: "player_actions",
			entrypoint: "on_pre_update",
			calldata: [pixelUpdate, appCaller, playerCaller],
	...
	const player_actions_onPreUpdate = async (snAccount: Account | AccountInterface, pixelUpdate: models.PixelUpdate, appCaller: models.App, playerCaller: string) => {
		try {
			return await provider.execute(
				snAccount,
				build_player_actions_onPreUpdate_calldata(pixelUpdate, appCaller, playerCaller),
				"pixelaw",
			);
		} catch (error) {
			console.error(error);

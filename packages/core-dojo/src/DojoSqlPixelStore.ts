		...
		if (!DojoSqlPixelStore.instance) {
			...
			if (typeof window !== "undefined" && Object.keys(window).length !== 0) {
				...
				const workerUrl = new URL(
					"./DojoSqlPixelStore.webworker.js",
					import.meta.url,
				);
				DojoSqlPixelStore.instance.worker = new Worker(workerUrl, {
					type: "module",
				});
				console.log("worker");
			} else {
			...
			DojoSqlPixelStore.instance.worker.onmessage =
				DojoSqlPixelStore.instance.handleRefreshWorker.bind(
